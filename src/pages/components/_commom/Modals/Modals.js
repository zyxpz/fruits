/**
 * 借助全局变量_global.APIS增加对路由切换的管理
 */
import React, { Component, createElement, cloneElement } from 'react';
import ReactDOM from 'react-dom';
import PropTypes from 'prop-types';
// import { hackForiOS11 } from '@common/js/utils/cssHack';
import './Modals.scss';
...
class Modals extends Component {
	...
	render() {
		...
		return (
			<div className={`common-modal ${className}`} style={{ ...style }}>
				...
				<div className="__fixed __enter" ref="fixed">
					<p className={`__close ${title && `__bb`}`}>
						{typeof title === "object" ? title : (title && <div className="__title">{title}</div>)}
						{showClose && <i className="iconfont icon-close" onClick={this.handleClose} />}
					</p>
